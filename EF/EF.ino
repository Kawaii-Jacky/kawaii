...
static bool jbool(const String&j,const char*k,bool d){String v=jstr(j,k);if(v.length())return v=="true"||v=="1";return jnum(j,k,d?1:0)!=0;}
...
static void startMqtt(){if(mqttClient||WiFi.status()!=WL_CONNECTED)return;esp_mqtt_client_config_t c={};c.broker.address.uri=MQTT_URI;c.credentials.client_id=MQTT_CLIENT_ID;c.credentials.username=MQTT_USERNAME;c.credentials.authentication.password=MQTT_PASSWORD;c.session.keepalive=60;c.broker.verification.crt_bundle_attach=esp_crt_bundle_attach;mqttClient=esp_mqtt_client_init(&c);if(mqttClient){esp_mqtt_client_register_event(mqttClient,MQTT_EVENT_ANY,mqttEvent,nullptr);esp_mqtt_client_start(mqttClient);}}
#else
...
static void startMqtt(){if(mqttClient||WiFi.status()!=WL_CONNECTED)return;esp_mqtt_client_config_t c={};c.uri=MQTT_URI;c.client_id=MQTT_CLIENT_ID;c.username=MQTT_USERNAME;c.password=MQTT_PASSWORD;c.keepalive=60;c.crt_bundle_attach=esp_crt_bundle_attach;c.event_handle=mqttEvent;mqttClient=esp_mqtt_client_init(&c);if(mqttClient)esp_mqtt_client_start(mqttClient);}
#endif
...
static void telemetry(){if(!mqttConnected||millis()-lastTelemetry<MQTT_INTERVAL_MS)return;lastTelemetry=millis();String p="{\"device\":\"ef-001\",\"humidity\":"+String(receivedHumidity)+",\"servo\":"+String(servoState?"true":"false")+",\"led\":"+String(ledState?"true":"false")+",\"heater\":"+String(heaterState?"true":"false")+",\"angle\":"+String(currentServoPosition)+"}";esp_mqtt_client_publish(mqttClient,MQTT_TELEMETRY,p.c_str(),p.length(),MQTT_QOS,0);}
