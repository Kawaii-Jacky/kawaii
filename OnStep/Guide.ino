  ...
  if (guideLst != guideSiderealTimer) {
    ...
    if (guideDirAxis1) {
      if (!inbacklashAxis1) {
        ...
        //脉冲导星时间：guideTimeRemainingAxis1
...
CommandErrors startGuideAxis1(char direction, int guideRate, long guideDuration) {
  // Check state
  if (faultAxis1)                       return CE_SLEW_ERR_HARDWARE_FAULT;//硬件故障
  if (!axis1Enabled)                    return CE_SLEW_ERR_IN_STANDBY;//轴1未启用
  if (parkStatus == Parked)             return CE_SLEW_ERR_IN_PARK;//已驻车
  if (trackingSyncInProgress())         return CE_MOUNT_IN_MOTION;//跟踪同步正在进行
  if (trackingState == TrackingMoveTo)  return CE_MOUNT_IN_MOTION;//跟踪移动到
  if (direction == guideDirAxis1)       return CE_NONE;//没有错误
  
  if (guideRate < 3) deactivateBacklashComp(); else reactivateBacklashComp();//如果guideRate小于3，则禁用反冲补偿，否则重新激活反冲补偿
  enableGuideRate(guideRate);//启用导星率
  guideDirAxis1=direction;//设置导星方向
  guideTimeThisIntervalAxis1=micros();//设置导星时间
  guideTimeRemainingAxis1=guideDuration*1000L;//设置导星时间
  cli();
  if (guideDirAxis1 == 'e') guideTimerRateAxis1=-guideTimerBaseRateAxis1; else guideTimerRateAxis1=guideTimerBaseRateAxis1; //如果guideDirAxis1为e，则设置guideTimerRateAxis1为-guideTimerBaseRateAxis1，否则设置为guideTimerBaseRateAxis1
  sei();
  return CE_NONE;//没有错误
}
...
//commanderrors是一个枚举，定义的函数类型可以有返回值，返回值表示错误类型，CE_NONE表示没有错误，而void表示没有返回值的函数类型。
...
CommandErrors startGuideAxis2(char direction, int guideRate, long guideDuration, bool absolute) {
  if (faultAxis2)                          return CE_SLEW_ERR_HARDWARE_FAULT;//硬件故障
  if (!axis1Enabled)                       return CE_SLEW_ERR_IN_STANDBY;//轴1未启用
  if (parkStatus == Parked)                return CE_SLEW_ERR_IN_PARK;//已驻车
  if (trackingSyncInProgress())            return CE_MOUNT_IN_MOTION;//跟踪同步正在进行
  if (trackingState == TrackingMoveTo)     return CE_MOUNT_IN_MOTION;//跟踪移动到
  if (direction == guideDirAxis2)          return CE_NONE;//没有错误
#if AXIS2_TANGENT_ARM == ON
  if (direction == 'n' && !guideNorthOk()) return CE_SLEW_ERR_OUTSIDE_LIMITS;//超出限制
  if (direction == 's' && !guideSouthOk()) return CE_SLEW_ERR_OUTSIDE_LIMITS;//超出限制
#endif

  enableGuideRate(guideRate);//启用导星率
  if (guideRate < 3) deactivateBacklashComp(); else reactivateBacklashComp();//如果guideRate小于3，则禁用反冲补偿，否则重新激活反冲补偿
  guideDirAxis2=direction;//设置导星方向
  guideTimeThisIntervalAxis2=micros();//设置导星时间
  guideTimeRemainingAxis2=guideDuration*1000L;//设置导星时间
  if (guideDirAxis2 == 's') { cli(); guideTimerRateAxis2=-guideTimerBaseRateAxis2; sei(); } //如果guideDirAxis2为s，则设置guideTimerRateAxis2为-guideTimerBaseRateAxis2，否则设置为guideTimerBaseRateAxis2
  if (guideDirAxis2 == 'n') { cli(); guideTimerRateAxis2= guideTimerBaseRateAxis2; sei(); } //如果guideDirAxis2为n，则设置guideTimerRateAxis2为guideTimerBaseRateAxis2，否则设置为guideTimerBaseRateAxis2
  if (!absolute && (getInstrPierSide() == PierSideWest)) { cli(); guideTimerRateAxis2=-guideTimerRateAxis2; sei(); } //如果absolute为false，并且getInstrPierSide()为PierSideWest，则设置guideTimerRateAxis2为-guideTimerRateAxis2，否则设置为guideTimerRateAxis2
