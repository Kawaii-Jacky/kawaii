...
void TGeoAlign::model(int n) {
  ...
  if (busy) return;                                                             // 忙
  if (n > 0) { numStars=n; return; }                                            // 命令
  if (numStars > 0) { busy=true; autoModel(numStars); busy=false; numStars=0; } // 等待解决
}

// 返回要添加到请求的RA,Dec以获得实际到达的RA,Dec的校正
...
// 望远镜镜筒或者光轴与赤纬轴不垂直：declination axis offset
  DO1 =_deo*sf;
...
void TGeoAlign::do_search(double sf, int p1, int p2, int p3, int p4, int p5, int p6, int p7, int p8, int p9)
{
  ...
  _deo_m,_deo_p,//declination axis offset 偏移搜索范围---m：最小值，p：最大值
  _pd_m,_pd_p,//polar direction error 偏移搜索范围---m：最小值，p：最大值
  _pz_m,_pz_p,//polar up-down error 偏移搜索范围---m：最小值，p：最大值
  _pe_m,_pe_p,//polar left-right error 偏移搜索范围---m：最小值，p：最大值
  _df_m,_df_p,//axis flex 轴弯曲偏移搜索范围---m：最小值，p：最大值
  _tf_m,_tf_p,//temperature flex 温度偏移搜索范围---m：最小值，p：最大值
  _ff_m,_ff_p,//fork flex 叉式偏移搜索范围---m：最小值，p：最大值
  _oh_m,_oh_p,//OH: offset HA 偏移搜索范围---m：最小值，p：最大值
  _od_m,_od_p,//OD：offset declination 偏移搜索范围---m：最小值，p：最大值
  
  _deo,_pd,_pz,_pe, _df,_tf,_ff, _ode,_ohe;//极轴偏移，极轴方向偏移，极轴左右偏移，极轴上下偏移，轴弯曲偏移，光轴偏移，叉子弯曲偏移，轴1偏移，轴2偏移

  double sf1=sf/(3600.0*Rad);//sf：search facotor 搜索步长因子（控制精度），3600.0：度转秒，Rad：弧度转度
  // 搜索
  // 设置参数空间
  ...
  for (_ode=_od_m; _ode <= _od_p; _ode++) {
    ode=((double)_ode)*sf1;//ode：east offset 偏移   odw：west offset 偏移  sf1：步长到弧度转化因子
    odw=-ode;
    ohe=((double)_ohe)*sf1;
    ohw=ohe;
...
void TGeoAlign::autoModel(int n) {
  ...
  best_dist   =3600.0*180.0;//最佳总偏移
  best_deo    =0.0;//最佳极轴偏移
  best_pd     =0.0;//极轴方向偏移（polar direction error）：赤纬轴相对于理想赤纬轴的偏差。
  best_pz     =0.0;//极轴上下偏移
  best_pe     =0.0;//极轴左右偏移
  best_tf     =0.0;//光轴偏移
  best_ff     =0.0;//叉式偏移
  ...
  ohe=ohe/num; best_ohe=round(ohe*Rad*3600.0);//Rad：弧度转度，3600.0：度转秒
  best_ohw=best_ohe; // 设置最佳RA偏移

#if MOUNT_TYPE == FORK//叉式赤道仪
  Ff=1; Df=0;
#else
  Ff=0; Df=1;//德式赤道仪和其他
#endif
