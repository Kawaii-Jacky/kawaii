...
void TGeoAlignH::init() {
  ...
  ax1Cor=0;  // 对齐内部索引为轴1
  ax2Cor=0;  // 对齐内部索引为轴2
  altCor=0;  // 几何坐标校正/对齐，- 低于天顶，+ 高于天顶
  azmCor=0;  // - 在天顶右边，+ 在天顶左边
  doCor =0;  // 赤纬轴/光学正交校正
  pdCor =0;  // 赤纬轴/方位正交校正
  dfCor =0;  // 赤纬轴轴弯曲
  tfCor =0;  // 管弯曲
...
// I=1 for 1st star, I=2 for 2nd star, I=3 for 3rd star  I=1 表示第一个星，I=2 表示第二个星，I=3 表示第三个星
// N=total number of stars for this align (1 to 9)  N=总星数（1到9）
// RA, Dec (all in degrees) 赤经，赤纬（所有以度为单位）  
CommandErrors TGeoAlignH::addStar(int I, int N, double RA, double Dec) {
  ...
  mount[I-1].azm=getInstrAxis1();//获取仪器轴1
  mount[I-1].alt=getInstrAxis2();//获取仪器轴2
  horToEqu(mount[I-1].alt,mount[I-1].azm,&mount[I-1].ha,&mount[I-1].dec);//水平到赤道
  mount[I-1].azm=mount[I-1].azm/Rad;//方位除以弧度
  mount[I-1].alt=mount[I-1].alt/Rad;//高度除以弧度
  mount[I-1].ha=degRange(mount[I-1].ha)/Rad;//赤经除以弧度
  mount[I-1].dec=mount[I-1].dec/Rad;//赤纬除以弧度

  actual[I-1].ha =haRange(LST()*15.0-RA);//赤经范围
  actual[I-1].dec=Dec;//赤纬
  equToHor(actual[I-1].ha,actual[I-1].dec,&actual[I-1].alt,&actual[I-1].azm);//赤道到水平
  actual[I-1].alt=actual[I-1].alt/Rad;//高度除以弧度
  actual[I-1].azm=actual[I-1].azm/Rad;//方位除以弧度
  actual[I-1].ha =degRange(actual[I-1].ha)/Rad;//赤经范围除以弧度
  actual[I-1].dec=actual[I-1].dec/Rad;//赤纬除以弧度

  if (getInstrPierSide() == PierSideWest) { actual[I-1].side=-1; mount[I-1].side=-1; } else//如果仪器赤纬轴在西方，则实际方位为-1
  if (getInstrPierSide() == PierSideEast) { actual[I-1].side=1; mount[I-1].side=1; } else { actual[I-1].side=0; mount[I-1].side=0; }//如果仪器赤纬轴在东方，则实际方位为1
...
// ------------------------------------------------------------
  // 由于管/光学不垂直于赤纬轴的失准
  // 负数是进一步（S）从北天极，摆动到赤道，赤纬的影响为0。在南天极，它成为（N）偏移。 不变与子午线翻转。  
  DO1 =_deo*sf;
  // 在HA上工作。 子午线翻转影响HA
...
// ------------------------------------------------------------
  // 失准，赤纬dec轴相对于极轴
  // 表示为极轴指向的校正
  // 负数是进一步（S）从北天极，摆动到赤道，赤纬的影响为0。在南天极，它成为（S）偏移。
  PD  =_pd*sf;
  // 在HA上工作。 子午线翻转影响HA
