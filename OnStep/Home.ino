...
CommandErrors goHome(boolean fast) {
  ...
  abortTrackingState=trackingState;//停止跟踪

  #if AXIS2_TANGENT_ARM == ON
    double h=getInstrAxis1();//获取仪器轴1  
    double i2=indexAxis2;//获取仪器轴2
    int p=getInstrPierSide();//获取仪器赤纬轴方向
    if (latitude >= 0) { if (p == PierSideWest) i2=180.0-i2; } else { if (p == PierSideWest) i2=-180.0-i2; }//如果纬度大于0，则将仪器轴2的值设置为180度减去仪器轴2的值，否则将仪器轴2的值设置为-180度减去仪器轴2的值
    e=goTo(h,i2,h,i2,p);//移动到仪器轴1和仪器轴2的位置
  #else
    trackingState=TrackingNone;//设置跟踪状态为None
    e=goTo(homePositionAxis1,homePositionAxis2,homePositionAxis1,homePositionAxis2,PierSideEast);//移动到仪器轴1和仪器轴2的位置
  #endif

  if (e == CE_NONE) homeMount=true; else trackingState=abortTrackingState;//如果移动成功，则设置homeMount为true，否则设置跟踪状态为停止跟踪 
...
// 设置望远镜的home位置；用户手动移动到赤经90度和赤纬90度（CWD位置），
// 然后第一个gotoEqu将设置赤纬轴方向并打开跟踪
CommandErrors setHome() {
  if (isSlewing()) return CE_MOUNT_IN_MOTION;//如果正在跟踪，则返回CE_MOUNT_IN_MOTION

  // 回到启动状态
  reactivateBacklashComp();//回退补偿
  initStartupValues();//初始化启动值
  currentAlt=45.0;//当前高度
  doFastAltCalc(true);//快速计算高度
  safetyLimitsOn=true;//安全限位

  // initialize and disable the stepper drivers
  StepperModeTrackingInit();//初始化步进模式跟踪
 
  // not parked, but don't wipe the park position if it's saved - we can still use it
  parkStatus=NotParked;//未停车
  nv.write(EE_parkStatus,parkStatus);//写入停车状态
  
  // reset PEC, unless we have an index to recover from this
  pecRecorded=nv.read(EE_pecRecorded);//读取PEC记录
  #if PEC_SENSE == OFF
    pecStatus=IgnorePEC;//忽略PEC
    nv.write(EE_pecStatus,pecStatus);//写入PEC状态
  #else
    pecStatus=nv.read(EE_pecStatus);//读取PEC状态
  #endif
  if (!pecRecorded) pecStatus=IgnorePEC;//如果PEC记录为0，则忽略PEC

  // the polar home position
  InitStartPosition();//初始化启动位置
  
  return CE_NONE;//返回CE_NONE
}
