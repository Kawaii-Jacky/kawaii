...
void pec() {
  ...
  if (pecIndex1 != lastPecIndex) {
    ...
    if (pecStatus == RecordPEC) {
      ...
      // 将accPecGuideHA转换为double，然后四舍五入为整数，然后限制在-StepsPerSecondAxis1到StepsPerSecondAxis1之间，然后限制在-127到127之间，然后如果pecFirstRecord为false，则将l设置为l+((int)pecBuffer[pecIndex1]-128)*2)/3，然后保存到pecBuffer[pecIndex1]中，然后从accPecGuideHA中减去l
