...
int getInstrPierSide() {
  if (atHome) return PierSideNone;//如果在家，则返回None
  cli(); long p2=posAxis2; sei();//获取仪器轴2  

#if AXIS2_TANGENT_ARM_CORRECTION == ON
  p2=atan(((double)p2/AXIS2_STEPS_PER_DEGREE)/Rad)*Rad*AXIS2_STEPS_PER_DEGREE;//计算仪器轴2的弧度
#endif
  
  double q=(double)((long)p2+indexAxis2Steps)/(double)AXIS2_STEPS_PER_DEGREE;//计算仪器轴2的弧度
  if ((q < -90.0) || (q > 90.0)) return PierSideWest; else return PierSideEast;//如果仪器轴2的弧度小于-90度或大于90度，则返回PierSideWest，否则返回PierSideEast
}
