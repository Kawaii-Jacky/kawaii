...
#define LED 255             // PCB 未将状态 LED 接到 U33，禁用 GPIO 控制
#define FAN 14              // PCB: 风扇低端 MOSFET(U28) 栅极由 GPIO14 控制
#define INA1_ALERT 255      // PCB 未连接 INA226 ALERT，禁用
#define INA2_ALERT 255      // PCB 未连接 INA226 ALERT，禁用
#define TempSensor 34       // PCB: CN10 NTC 信号连接 GPIO34
...
void mpptSetup() {
  ...
  if (LED < 40) pinMode(LED, OUTPUT);
  pinMode(FAN, OUTPUT);
  pinMode(TempSensor, INPUT);
  if (INA1_ALERT < 40) pinMode(INA1_ALERT, INPUT);
  if (INA2_ALERT < 40) pinMode(INA2_ALERT, INPUT);
