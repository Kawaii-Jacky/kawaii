...
void System_Processes(){
  ...
  if(currentSystemProcessesMillis-prevSystemProcessesMillis>=millisRoutineInterval){                                           //每 millisRoutineInterval (ms) 运行例程
    ...
    if(millis() - lastSaveTime >= 300000) { // 5分钟 = 300000毫秒
      EEPROM.put(MPPT_EEPROM_BASE + 100, totalEnergy);
      EEPROM.commit();
      lastSaveTime = millis();
    }
...
void factoryReset(){
  ...
  for(int i = 0; i < 200; i++) {
    EEPROM.write(MPPT_EEPROM_BASE + i, 0xFF);  // 写入0xFF来清除数据
  }
  EEPROM.commit();
  
  // 然后写入默认值
  EEPROM.write(MPPT_EEPROM_BASE + 0,1);  //存储: 充电算法（1 = MPPT 模式）
  EEPROM.write(MPPT_EEPROM_BASE + 2,1);  //存储: 输出模式（1 = 充电器模式）
  ...
  EEPROM.put(MPPT_EEPROM_BASE + 3, defaultVoltageMax);    // 存储最大电池电压（浮点数，地址3）
  EEPROM.put(MPPT_EEPROM_BASE + 7, defaultVoltageMin);    // 存储最小电池电压（浮点数，地址7）
  EEPROM.put(MPPT_EEPROM_BASE + 30, defaultCurrent);      // 存储充电电流（浮点数，地址30）
  // 存储发送间隔默认值（5秒 = 5000毫秒）
  int defaultSendingInterval = 5000;
  EEPROM.put(MPPT_EEPROM_BASE + 20, defaultSendingInterval);                // 存储发送间隔（毫秒）
  EEPROM.write(MPPT_EEPROM_BASE + 25,1); //存储: 启用自动加载（默认开启，新地址25）
  EEPROM.write(MPPT_EEPROM_BASE + 13,1); //存储: 风扇启用 (Bool)
  EEPROM.write(MPPT_EEPROM_BASE + 14,60); //存储: 风扇温度（整数）
  EEPROM.write(MPPT_EEPROM_BASE + 15,90); //存储: 关机温度（整数）
  EEPROM.write(MPPT_EEPROM_BASE + 16,1); //存储: 启用 WiFi（布尔值）
  EEPROM.write(MPPT_EEPROM_BASE + 18,0); //存储: 风扇手动控制（默认值：0 = 自动模式）
  EEPROM.commit();
  loadSettings();
}
...
void loadSettings(){ 
  MPPT_Mode          = EEPROM.read(MPPT_EEPROM_BASE + 0);                       // 加载保存的充电模式设置
  output_Mode        = EEPROM.read(MPPT_EEPROM_BASE + 2);                       // 加载保存的输出模式设置
  // 使用EEPROM.get()读取浮点数，确保数据类型正确
  EEPROM.get(MPPT_EEPROM_BASE + 3, voltageBatteryMax);                          // 加载保存的最大电池电压设置（地址3）
  EEPROM.get(MPPT_EEPROM_BASE + 7, voltageBatteryMin);                          // 加载保存的最低电池电压设置（地址7）
  EEPROM.get(MPPT_EEPROM_BASE + 30, currentCharging);                           // 加载保存的充电电流设置（地址30）
  EEPROM.get(MPPT_EEPROM_BASE + 20, Sending_Interval);                          // 加载发送间隔
  EEPROM.get(MPPT_EEPROM_BASE + 108, PWM_MaxDC);                                // 加载保存的PWM_MaxDC设置（地址108）
  EEPROM.get(MPPT_EEPROM_BASE + 112, pwmMaxLimited);                            // 加载保存的pwmMaxLimited设置（地址112）

  // 加载基本设置
  enableFan          = EEPROM.read(MPPT_EEPROM_BASE + 13);                      // 加载保存的风扇启用设置
  temperatureFan     = EEPROM.read(MPPT_EEPROM_BASE + 14);                      // 加载保存的风扇温度设置
  temperatureMax     = EEPROM.read(MPPT_EEPROM_BASE + 15);                      // 加载保存的关机温度设置
  enableWiFi         = EEPROM.read(MPPT_EEPROM_BASE + 16);                      // 加载保存的 WiFi 启用设置  
  flashMemLoad       = EEPROM.read(MPPT_EEPROM_BASE + 25);                      // 加载保存的闪存自动加载功能（新地址25）
  overrideFan        = EEPROM.read(MPPT_EEPROM_BASE + 18);                      // 加载保存的风扇手动控制设置
  telemCounterReset  = EEPROM.read(MPPT_EEPROM_BASE + 19);                      // 加载保存的数据重置周期
  
  // 加载发电量数据
  EEPROM.get(MPPT_EEPROM_BASE + 100, totalEnergy);                              // 加载总发电量
  EEPROM.get(MPPT_EEPROM_BASE + 104, lastDayReset);                             // 加载上次日重置时间戳
...
void saveSettings(){
  ...
  EEPROM.write(MPPT_EEPROM_BASE + 0,MPPT_Mode);           //存储：算法 
  EEPROM.write(MPPT_EEPROM_BASE + 2,output_Mode);         //存储：输出模式（地址2）
  // 使用EEPROM.put()存储浮点数，确保数据类型正确
  EEPROM.put(MPPT_EEPROM_BASE + 3, voltageBatteryMax);    // 存储最大电池电压（浮点数，地址3）
  EEPROM.put(MPPT_EEPROM_BASE + 7, voltageBatteryMin);    // 存储最小电池电压（浮点数，地址7）
  EEPROM.put(MPPT_EEPROM_BASE + 30, currentCharging);     // 存储充电电流（浮点数，地址30）
  EEPROM.put(MPPT_EEPROM_BASE + 20, Sending_Interval);    // 保存发送间隔
  
  EEPROM.write(MPPT_EEPROM_BASE + 13,enableFan);          //风扇启用
  EEPROM.write(MPPT_EEPROM_BASE + 14,temperatureFan);     //风扇温度
  EEPROM.write(MPPT_EEPROM_BASE + 15,temperatureMax);     //关机温度
  EEPROM.write(MPPT_EEPROM_BASE + 16,enableWiFi);         //启用 WiFi
//EEPROM.write(MPPT_EEPROM_BASE + 17,flashMemLoad);       //启用自动加载（必须排除批量保存，酌情取消注释）
  EEPROM.write(MPPT_EEPROM_BASE + 18,overrideFan);        //存储：风扇手动控制
  EEPROM.write(MPPT_EEPROM_BASE + 19,telemCounterReset);  //存储：数据重置周期
  
  // 保存发电量数据
  EEPROM.put(MPPT_EEPROM_BASE + 100, totalEnergy);                              // 保存总发电量
  EEPROM.put(MPPT_EEPROM_BASE + 104, lastDayReset);                             // 保存上次日重置时间戳
  
  // 保存PWM_MaxDC设置（地址108）
  EEPROM.put(MPPT_EEPROM_BASE + 108, PWM_MaxDC);
  // 保存pwmMaxLimited设置（地址112）
  EEPROM.put(MPPT_EEPROM_BASE + 112, pwmMaxLimited);
  
  EEPROM.commit();                     //将设置更改保存到闪存
}
void saveAutoloadSettings(){
  EEPROM.write(MPPT_EEPROM_BASE + 25,flashMemLoad);       //STORE: 启用自动加载（使用地址25，与loadSettings一致）
  EEPROM.commit();                     //将设置更改保存到闪存
}
void initializeFlashAutoload(){
  if(disableFlashAutoLoad==0){
    flashMemLoad = EEPROM.read(MPPT_EEPROM_BASE + 25);       //加载保存的自动加载（新地址25）
