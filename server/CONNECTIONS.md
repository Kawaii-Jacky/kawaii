...
网页账户和 MQTT 控制器账户是两套独立身份。普通用户或操作员登录后，可在“连接设置”
提交一个三设备套组申请；管理员在 `/admin` 审核后，一键创建套组并自动写入
`user_controller_access`，因此申请账户和实际硬件套组的对应关系由审批结果确定。
每个套组固定包含主控 `esp32-001`、能源 `mppt-001` 和平场板 `ef-001` 三台设备：

- 所有角色默认都没有设备权限，管理员角色也不能绕过套组隔离；
- 每个账户只能使用被明确批准的一整套三台设备；
- 设备列表、最新值、历史遥测、告警、命令查询、命令下发和 SSE 实时事件均按
  同一授权关系过滤；
- 未授权设备统一返回 `404`，避免通过接口枚举设备；
- 授权发生变化时，该用户已有会话会被撤销，需要重新登录后才可使用新权限。

登录用户不能自行认领其他套组，也不会在申请阶段输入控制器 MQTT 账号。审批成功后，
连接设置会显示已绑定的套组、三台设备的头文件配置和 MQTT 密码，用户可以下载并写入
对应硬件；服务端仍通过统一 HTTPS 访问，浏览器不直接连接 MQTT。管理员在 `/admin`
审核申请后，服务端自动生成套组 ID、Backend Controller 凭据、三台设备各自的凭据和
Topic 命名空间，写入 Mosquitto 密码库与 ACL，并重启 API。
