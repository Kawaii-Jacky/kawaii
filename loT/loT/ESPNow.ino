
// ==================== ESP-NOW初始化函数 ====================
void initEspNow() {
  // 设置WiFi模式为Station
  WiFi.mode(WIFI_STA);
  
  // 打印本机MAC地址
  uint8_t mac[6];
  WiFi.macAddress(mac);
  Serial.printf("IoT板子MAC地址: %02X:%02X:%02X:%02X:%02X:%02X\n", 
                mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
  
  // 初始化ESP-NOW
  if (esp_now_init() == ESP_OK) {
    Serial.println("ESP-NOW 初始化成功");
    
    // 添加平场板对等设备（使用动态MAC地址）
    esp_now_peer_info_t peerInfo = {};
    memcpy(peerInfo.peer_addr, flatFieldMac, 6);
    peerInfo.channel = 0;
    peerInfo.encrypt = false;
    const esp_err_t peerResult = esp_now_add_peer(&peerInfo);
    if (peerResult != ESP_OK && peerResult != ESP_ERR_ESPNOW_EXIST) {
      Serial.printf("ESP-NOW peer add failed: %d\n", (int)peerResult);
    }
    Serial.printf("ESP-NOW连接平场板MAC: %02X:%02X:%02X:%02X:%02X:%02X\n", 
                  flatFieldMac[0], flatFieldMac[1], flatFieldMac[2], 
                  flatFieldMac[3], flatFieldMac[4], flatFieldMac[5]);
    
    // Sending is scheduled from the main loop; timer callbacks must not perform
    // Wi-Fi/ESP-NOW work or Serial I/O.
  } else {
    Serial.println("ESP-NOW 初始化失败");
  }
}

// 统一的ESP-NOW数据发送函数
void sendEspNowData() {
  // 发送湿度数据到平场板
  EspNowHumiConfig config;
  config.humiThreshold = (int)dhtHumidity; // 发送实际湿度值
  
  esp_err_t result1 = esp_now_send(flatFieldMac, (uint8_t*)&config, sizeof(config));
  if (result1 == ESP_OK) {
    Serial.printf("已通过ESP-NOW发送实际湿度值到平场板: %.1f%%\n", dhtHumidity);
  } else {
    Serial.println("ESP-NOW发送实际湿度值失败");
  }
  

}

void handleEspNowPeriodic() {
  static unsigned long lastSend = 0;
  const unsigned long now = millis();
  if (now - lastSend >= ESPNOW_SEND_INTERVAL) {
    lastSend = now;
    sendEspNowData();
  }
}




// ESP-NOW数据发送间隔设置回调

