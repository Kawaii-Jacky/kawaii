...
static void mqttPublishAck(const String& id, const String& command, bool ok, const String& error = String()) {
  if (!mqttConnected || mqttClient == nullptr || id.isEmpty()) return;
  String payload = String("{\"schema\":1,\"device\":\"") + DEVICE_MQTT_CLIENT_ID +
    "\",\"id\":\"" + escapeJson(id) + "\",\"command\":\"" + escapeJson(command) +
    "\",\"ok\":" + (ok ? "true" : "false");
  if (!error.isEmpty()) payload += String(",\"error\":\"") + escapeJson(error) + "\"";
  payload += "}";
  esp_mqtt_client_publish(mqttClient, DEVICE_MQTT_TOPIC_REPORTED, payload.c_str(), payload.length(), DEVICE_MQTT_QOS, 0);
}

static bool handleMqttCommand(const String& json, String& error) {
  ...
  else if (command == "fan_threshold") { long value=jsonLong(json,"value",-1); if(value<20||value>80){error="fan threshold out of range";return false;} setFanTempThreshold((uint8_t)value); }
  else if (command == "camera_timer") { long minutes=jsonLong(json,"minutes",-1);if(minutes<1||minutes>1439){error="camera timer out of range";return false;} cameraAutoOffTime = (unsigned long)minutes * 60000UL; if (cameraPowerState) cameraPowerStartTime = millis(); }
  else if (command == "camera") setCameraPower(jsonBool(json, "state", false));
  else if (command == "bluetooth") { bool connect=jsonBool(json,"state",false);if(connect){pairBluetooth();if(!btConnected){error="OnStep Bluetooth connection failed";return false;}}else disconnectBluetooth(); }
  ...
  else if (command == "onstep") { int action = jsonLong(json, "action", 0);if(!btConnected){error="OnStep Bluetooth is not connected";return false;} if (action == 1) sendDateTimeToOnStep(); else if (action == 2) sendParkCommand(); else if (action == 3) sendUnparkCommand(); else if (action == 4) sendHomeCommand(); else if (action == 5) setCurrentPositionAsHome(); else if (action == 6) setCurrentPositionAsPark(); else {error="unsupported OnStep action";return false;} }
  else if (command == "debug") sendAllDebugInfo();
  else if (command == "terminal") processTerminalCommand(jsonString(json, "value"));
  else { error=String("unknown command: ")+command;mqttLog(error);return false; }
...
static void processMqttEvent(esp_mqtt_event_handle_t event) {
  if (event->event_id == MQTT_EVENT_CONNECTED) {
    ...
    if (topic == DEVICE_MQTT_TOPIC_COMMAND || topic == DEVICE_MQTT_TOPIC_DESIRED) {
      String error;
      String command = jsonString(payload, "command");
      bool ok = handleMqttCommand(payload, error);
      mqttPublishAck(jsonString(payload, "id"), command, ok, error);
    }
...
void publishTelemetry() {
  if (!mqttConnected || mqttClient == nullptr) return;
  char payload[768];
  const char* roofState = (motorForwardState || motorReverseState) ? "moving" : "unknown";
  const int n = snprintf(payload, sizeof(payload),
    "{\"device\":\"%s\",\"dht_temperature\":%.1f,\"dht_humidity\":%.1f,\"utc_temperature\":%.1f,\"output_voltage\":%.2f,\"output_current\":%.2f,\"power_output\":%.2f,\"rain_analog\":%u,\"rain_detected\":%s,\"heater\":%s,\"heater_mode\":%s,\"fan\":%s,\"fan_mode\":%s,\"fan_threshold\":%u,\"mosfet\":%s,\"camera\":%s,\"cameraDurationMinutes\":%lu,\"bluetooth\":%s,\"roof\":\"%s\"}",
    DEVICE_MQTT_CLIENT_ID, dhtTemperature, dhtHumidity, utcTemperature, outputVoltage, outputCurrent, powerOutput,
    (unsigned)rainAnalogValue, rainDetected ? "true" : "false", heaterState ? "true" : "false", heaterAutoMode ? "true" : "false",
    fanState ? "true" : "false", fanAutoMode ? "true" : "false", (unsigned)fanTempThreshold,
    mosfetState ? "true" : "false", cameraPowerState ? "true" : "false", cameraAutoOffTime / 60000UL, btConnected ? "true" : "false", roofState);
